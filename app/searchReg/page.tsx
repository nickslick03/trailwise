"use client";

import Head from "next/head";
import Search from "@/components/Search";
import FilterComponent from "@/components/FilterComponent";
import BlackBoxPark from "@/components/BlackBoxPark";
//import NavigationBar from "@/components/NavigationBar";

export default function SearchMap() {
    return (
        <div className="fixed inset-0 flex items-center justify-center p-4 bg-gray-100">
            <Head>
                <title>Search Parks</title>
            </Head>

            <main className="max-w-md w-full mx-auto bg-white rounded-3xl shadow-xl overflow-hidden h-[90vh]">
                <div className="p-4">

                    <div className="flex items-center justify-between p-4 bg-white rounded-lg shadow-sm border border-gray-200 w-full max-w-md h-10">
                        {/* Text on the left */}
                        <span className="text-gray-700 font-medium">
                            Miles from Current Location
                        </span>
                        
                        {/* Dropdown on the right */}
                        <select 
                            className="bg-gray-50 border border-gray-300 text-gray-700 py-1 px-3 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 h-8"
                        >
                            <option value="10">10</option>
                            <option value="25">25</option>
                            <option value="50">50</option>
                            <option value="100">100</option>
                        </select>

                        
                    </div>

                    <br/>
                    <h1 className="text-gray-800 text-lg">Search By Regulation</h1>

                    {/* Scrollable BlackBox content area */}
                    <div className="flex-1 overflow-y-auto h-93">
                        <BlackBoxPark />
                        <BlackBoxPark />
                        <BlackBoxPark />
                        {/* Add more BlackBoxPark components as needed */}
                    </div>
                    
                    {/* Filter Component */}
                    <div className="sticky bottom-0 bg-white border-t">
                        <FilterComponent />
                    </div>
                    {/* <NavigationBar /> */}
                </div>
            </main>
        </div>
    )
}